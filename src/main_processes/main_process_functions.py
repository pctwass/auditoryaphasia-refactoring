...
def generate_meta_file(session_type: str):
    ...
    with open(
        os.path.join(
            system_config.data_dir, system_config.save_folder_name, "meta.json"
        ),
        "w",
    ) as f:
...
def create_and_start_subprocesses() -> tuple[
    ...
    audio_stim_process, audio_stim_state_dict = (
        process_manager.create_audio_stim_process(
            kwargs=dict(name="audio", name_main_outlet="main")
        )
    )
    ...
    visual_fb_process, visual_fb_state_dict = process_manager.create_visual_fb_process(
    ...
    ) = process_manager.create_acquisition_process(
        num_classes=classifier_config.n_class,
        kwargs=kwargs_acquisition,
        kwargs_live_barplot=kwargs_live_barplot,
    )
...
def open_audio_device(
    intermodule_comm_outlet: StreamOutlet, audio_stim_state_dict: dict[str, any]
):
...
def launch_acquisition(
    intermodule_comm_outlet: StreamOutlet, condition: str, soa: float
):
    intermodule_comm.send_cmd_LSL(intermodule_comm_outlet, "acq", "init")

    intermodule_comm.send_params_LSL(
        intermodule_comm_outlet, "acq", "condition", condition
    )
...
def gen_eeg_fname(
    ...
    f_name = utils.increment_file_name_if_exists(f_name + extension)
