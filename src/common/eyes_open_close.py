...
def run_eyes_open_close(outlet, pre_or_post):
    ...
    while True:
        f_save_eye = os.path.join(
            system_config.data_dir, system_config.save_folder_name, folder
        )
        if not os.path.exists(f_save_eye):
            os.makedirs(f_save_eye)
        input("Press Any Key to Start Eyes Open Recording (%ds)" % time_rec)

        f_name = utils.increment_file_name_if_exists(
            os.path.join(f_save_eye, "eyes_open.eeg")
        )
        ...
        f_name = utils.increment_file_name_if_exists(
            os.path.join(f_save_eye, "eyes_close.eeg")
        )
        # f_name = utils.check_file_exists_on_extenstion(
        ...
        user_input_repeat_procedure = input(
            "Do you want to record eyes open closed again? [y]/n : "
        )
        if (
