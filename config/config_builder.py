...
# directly
def build_configs(cfg_root: Path = Path("./config")):

    general_config_file_path = cfg_root.joinpath("config_general.yaml")
    system_config_file_path = cfg_root.joinpath("config_system.yaml")
    classifier_config_file_path = cfg_root.joinpath("config_classifier.yaml")

    general_config = build_general_config(general_config_file_path)
    build_system_config(system_config_file_path)
    build_classifier_config(classifier_config_file_path)


def build_general_config(config_file_path: str = Path("./config/config_general.yaml")):
...
def build_system_config(config_file_path: str = Path("./config/config_system.yaml")):
    ...
    if "datestr" in config_json_dict and not config_json_dict["datestr"]:
        ...
        system_config.datestr = datetime.strptime(config_json_dict["datestr"]).strftime(
            "%y_%m_%d"
        )
    else:
