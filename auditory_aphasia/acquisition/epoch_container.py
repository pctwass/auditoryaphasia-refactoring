...
class EpochContainer:
    def __init__(
        ...
        # the Old version did just append epochs one after the other, growing infinitely
        # we use the same for now
        self.epochs_list = []
        self.markers_list = []
        self.n_new_epochs = 0
        self.total_epoch_count = 0  # just for book keeping
    ...
    def update(
        ...
        ----------
        eeg_data : np.ndarray
            the latest eeg_data shape (n_samples, n_channels)
        eeg_ts: np.ndarray
            time stamps of the eeg data
        markers : np.ndarray
            shape (n_samples, )
        markers_ts : np.ndarray
            time stamps of the markers
        """
        # find the indices of the markers that we want to epoch
        markers_to_epoch_indices = np.where(np.isin(markers, self.markers_to_epoch))[0]
        markers = markers[markers_to_epoch_indices]
        m_ts = markers_ts[markers_to_epoch_indices]

        for ts, m in zip(m_ts, markers):
            # find closest time stamp in the EEG
            idx = np.argmin(np.abs(eeg_ts - ts))

            start_idx = idx + int(self.tmin * self.sample_freq)
            end_idx = idx + int(self.tmax * self.sample_freq)
            epoch_data = eeg_data[start_idx:end_idx, :]
            self.epochs_list.append(
                epoch_data.T
            )  # Transpose to get (n_epochs, n_channels, n_samples)

            self.markers_list.append(m)
            self.n_new_epochs += 1
            self.total_epoch_count += 1

    def has_new_data(self) -> bool:
        return self.n_new_epochs > 0

    def get_new_data(
        self,
    ) -> tuple[
        np.ndarray | None, np.ndarray | None
    ]:  # list objects are actually np.ndarrays

        if self.n_new_epochs:
            logger.debug(f"getting {self.n_new_epochs=} new data")
            logger.debug(f"getting {self.markers_list[-self.n_new_epochs:]=}")

            # # TODO: DOUBLE CHECK THIS IS HOW I FOUND THE CODE, THIS WOULD ALWAYS JUST RETURN A SINGLE EPOCH!
            # epochs_new = self.data[self.new_data_start_index, :, :]
            # events = self.events[self.new_data_start_index, 2]

            epochs_new = self.data[-self.n_new_epochs :]
            events = self.events[-self.n_new_epochs :]
...
class EpochContainerOld:
    def __init__(
        ...
        self.length_epoch_in_samples = (
            np.floor(sample_freq * (self.epoch_range[1] - self.epoch_range[0])).astype(
                np.int64
            )
            + 1
        )
    ...
    def get_new_data(
        self,
    ) -> tuple[list[float], list[int]]:  # list objects are actually np.ndarrays
    ...
    def update(self):
        if len(self.__marker.data) > len(self.epoched_marker):
            ...
            self.epoched_marker = np.append(
                self.epoched_marker, markers_to_epoch
            ).astype(np.int64)
        ...
        if unepoched_markers_indices.size != 0:
            ...
            for unepoched_marker_idx in unepoched_markers_indices:
                ...
                unepoched_marker_timestamp_eeg_onset_adjusted = (
                    unepoched_marker_timestamp + self.epoch_range[0]
                )  # (i.e. + self.tmin)
                ...
                indices_eeg_time_range_after_marker = np.where(
                    (self.__eeg.time - unepoched_marker_timestamp_eeg_onset_adjusted)
                    > 0
                )[0]

                if len(indices_eeg_time_range_after_marker) != 0:
                    idx_epoch_start_time = indices_eeg_time_range_after_marker[0]
                    idx_epoch_start_time = (
                        idx_epoch_start_time + self.length_epoch_in_samples
                    )  # shift by one epoch?

                    # check if the epoch completion time is a entry in the eeg timestamps
                    if len(self.__eeg.time) - 1 > idx_epoch_start_time:
                        indices_matched_epoch_start_times = np.append(
                            indices_matched_epoch_start_times, idx_epoch_start_time
                        )
            ...
            if indices_matched_epoch_start_times.size != 0:
                ...
                for idx_epoch_start_time in indices_matched_epoch_start_times:
                    idx_range_epoch_markers = (
                        self.epoch_range + self.__marker.time[idx_epoch_start_time]
                    )

                    indices_eeg_times_during_epoch = np.where(
                        self.__eeg.time - idx_range_epoch_markers[0] > 0
                    )[0]
                    idx_eeg_time_epoch_start = indices_eeg_times_during_epoch[0]
                    idx_eeg_time_epoch_end = (
                        idx_eeg_time_epoch_start + self.length_epoch_in_samples
                    )
                    idx_range_epoch = [
                        idx_eeg_time_epoch_start,
                        idx_eeg_time_epoch_end + self.length_epoch_in_samples,
                    ]

                    marker_timestamp_epoch_start = self.__marker.time[
                        idx_epoch_start_time
                    ]
                    indices_eeg_after_epoch_start_marker = np.where(
                        self.__eeg.time - marker_timestamp_epoch_start > 0
                    )[0]
                    idx_first_eeg_timestamp_after_marker = (
                        indices_eeg_after_epoch_start_marker[0]
                    )

                    data_tmp = self.__eeg.data[
                        :, idx_range_epoch[0] : idx_range_epoch[1]
                    ]

                    data_tmp = np.reshape(
                        data_tmp, (1, self.n_channels, self.length_epoch_in_samples)
                    )
                    ...
                    events = np.append(
                        ...
                        np.reshape(
                            [
                                idx_first_eeg_timestamp_after_marker,
                                0,
                                self.__marker.data[idx_epoch_start_time],
                            ],
                            (1, 3),
                        ),
                        axis=0,
