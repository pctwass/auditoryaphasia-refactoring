...
def gen_plan_play_oddball(
    ...
    audio_files.load(
        non_target_id,
        os.path.join(oddball_base, system_config.oddball_non_target),
        volume=volume,
    )
    audio_files.load(
        target_id,
        os.path.join(oddball_base, system_config.oddball_target),
        volume=volume,
    )
...
def gen_plan_play_word(
    ...
    load_audio_data(
        data, repo_base_dir, condition, words_to_speak, volume, load_sentences=False
    )
...
def gen_plan_spk_fam(
    repo_base_dir: str, condition: str, soa: float = 1, volume: float = 1
) -> tuple[list[any], pyscab.DataHandler]:
    data = pyscab.DataHandler()
...
def set_globals(familirization_args):
    ...
    if is_visual:
        intermodule_comm_outlet = (
            intermodule_comm.create_intermodule_communication_outlet(
                "main", channel_count=4, source_id="main"
            )
        )
...
def send_marker_visual(val: int):
    ...
    if is_visual:
        ...
        if val >= 200 and val <= 205:
            ...
            intermodule_comm.send_cmd_LSL(
                intermodule_comm_outlet, "visual", "highlight_speaker", spk_num
            )

        if val >= 101 and val <= 116:
            if speaker_showed:
                intermodule_comm.send_cmd_LSL(
                    intermodule_comm_outlet, "visual", "show_crosshair"
                )
                speaker_showed = False
...
def load_audio_data(
    ...
    for idx, word in enumerate(config.words):
        ...
        word_file_path = dir_navigator.get_word_audio_file_path(
            repo_base_dir, condition, word, file_name
        )
        data.load(idx + 1, word_file_path, volume=volume)

        if load_sentences:
            sentence_file_path = dir_navigator.get_sentence_audio_file_path(
                repo_base_dir, condition, word, file_name
            )
...
def run_familiarization(config: GeneralConfig, system_config: SystemConfig):
    ...
    if familirization_args.visual:
        ...
        visual_process, visual_fb_state_dict = process_manager.create_visual_fb_process(
            kwargs=dict(name="visual", name_main_outlet="main")
        )
    ...
    pyscab_audio_interface = pyscab.AudioInterface(
        ...
        format=system_config.audio_format,
    ...
    while is_familiarizing:
        try:
            ...
            match selected_fam_stage:
                ...
                case 4:
                    ...
                    if en_pitch_shift == "y":
                        ...
                        condition = "6d"
                    audio_plan, audio_data = gen_plan_spk_fam(
                        repo_base_dir,
                        condition,
                        soa=float(selected_soa),
                        volume=master_volume,
                    )
                ...
                case 5:
                    ...
                    selected_soa = input()
                    audio_plan, audio_data = gen_plan_hp_fam(
                        repo_base_dir,
            ...
            pyscab_stim_controller.play(
                audio_plan, audio_data, time_termination="auto", pause=0
            )
