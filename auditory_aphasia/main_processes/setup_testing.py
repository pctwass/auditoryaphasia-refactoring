...
def test_stereo(system_config: SystemConfig, volume=1, num_rep=20, ch=[7, 8], soa=1):
    data = pyscab.DataHandler()
    ...
    data.load(
        1,
        os.path.join(
            system_config.repository_dir_base, "media", "audio", "misc", "left.wav"
        ),
    )
    data.load(
        2,
        os.path.join(
            system_config.repository_dir_base, "media", "audio", "misc", "right.wav"
        ),
    )
...
def spk_volume_adjust(
    ...
    for m in range(0, 6):
        ...
        word_dir = os.path.join(
            base_dir, "words", condition, words[m], "%s.wav" % (str(1))
        )
    ...
    for m in range(0, 6):
        ...
        sentence_dir = os.path.join(
            base_dir, "sentences", condition, words[m], "%s.wav" % (str(1))
        )
...
def main_setup_testing(config: GeneralConfig, system_config: SystemConfig):
    ...
    while isfamiliarizing:
        ...
        if int(selected_fam_stage) == 1:
            ...
            audio_plan, audio_data = test_stereo(system_config, volume=MasterVolume)
        elif int(selected_fam_stage) == 3:
            audio_plan, audio_data = spk_volume_adjust(
                base_dir, words, volume=MasterVolume
            )
