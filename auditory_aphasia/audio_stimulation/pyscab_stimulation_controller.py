...
class PyscabStimulationController(object):
    ...
    def __init__(
        ...
        logger.debug(
            "time_tick for Stimulation Controller was set to %s", str(self.time_tick)
        )
    ...
    def marker_send_offset(self, val):
        t = threading.Thread(
            target=self.marker_send_offset_thread, kwargs=dict(val=val)
        )
        t.start()
        ...
        while self.state_dict["audio_status"] == AudioStatus.PLAYING:
            ...
            for idx, plan in enumerate(plans):
                if now > plan[0]:
                    ...
                    logger.debug(
                        f"Playing, id:{str(plan[1])}, ch:{str(plan[2])}, marker:{str(plan[3])}, path:f{data.get_path_by_id(plan[1])}"
                    )
