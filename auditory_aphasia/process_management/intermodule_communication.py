...
def create_intermodule_communication_outlet(
    ...
    channel_format: int | str = "string",
    source_id: str = "",
) -> StreamOutlet:
    # srate is set to IRREGULAR_RATE when nominal_srate = 0
    info = StreamInfo(
        "IntermoduleCommunication_" + module_name,
        type,
        channel_count,
        nominal_srate,
        channel_format,
...
def send_cmd_LSL(
    outlet: StreamOutlet, target_module: str, cmd: str, params: dict[str, any] = None
):
    # TODO: For Dareplane, this could be dealt with PCOMM callbacks
