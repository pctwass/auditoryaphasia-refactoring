...
def run_oddball(outlet, audio_state_dict, number_of_repetitions=None):
    ...
    while True:
        ...
        logger.debug(
            "oddball nT, id:%s, dir:%s, volume:%s",
            nT_id,
            os.path.join(oddball_base, system_config.oddball_non_target),
            config.master_volume,
        )
        ...
        logger.debug(
            "oddball number of repetitions : %s", str(number_of_repetitions_oddball)
        )
            ...
            if stim == 1:
                play_plan.append(
                    [
                        time_plan,
                        T_id,
                        system_config.oddball_channels,
                        system_config.oddball_marker["target"],
                    ]
                )
            else:
                play_plan.append(
                    [
                        ...
                        nT_id,
                        system_config.oddball_channels,
                        system_config.oddball_marker["nontarget"],
                    ]
                )
        ...
        for i in range(number_of_repetitions):
            ...
            while True:
                ...
                f_dir = os.path.join(
                    ...
                    "Oddball_%d.eeg" % oddball_run_idx,
                )
            ...
            logger.debug(
                "oddball_%d will be saved in : %s" % (oddball_run_idx, str(f_dir))
            )
        ...
        user_input_repeat_procedure = input(
            "Do you want to play oddball again? [y]/n : "
        )
        if (
            not user_input_repeat_procedure == ""
