...
class VisualFeedbackController():
    ...
    def show_screen(self):
        self.window = visual.Window(
            self.display_size,
            color=self.system_config.background_color,
            fullscr=self.fullscreen_mode,
            pos=self.window_position,
            units=self.window_units,
            screen=self.screen,
        )
        self.smiley = visual.ImageStim(
            win=self.window, size=self.system_config.size_smiley
        )
        self.barplot = visual.ImageStim(win=self.window)
        self.eyes_open = visual.ImageStim(
            win=self.window,
            image=os.path.join(self.images_dir_base, "eyesOpen.jpg"),
            size=self.system_config.size_eye_open_close,
        )
        self.eyes_close = visual.ImageStim(
            win=self.window,
            image=os.path.join(self.images_dir_base, "eyesClosed.jpg"),
            size=self.system_config.size_eye_open_close,
        )
        self.dancing_gif = visual.ImageStim(
            win=self.window, size=self.system_config.size_dancing_gif
        )
        self.crosshair = Crosshair(self.window)
        self.speakers = Speakers(
            self.window, highlight_color=self.system_config.highlight_color
        )
    ...
    def show_barplot_with_smiley(self):
        self.fb_smiley, self.fb_bar = self._show_pics_vert(
            win=self.window,
            img_1_dir=os.path.join(self.images_dir_base, "smiley_transparent.png"),
            img_2_dir=os.path.join(
                self.system_config.repository_dir_base, "media", "tmp", "fb_barplot.png"
            ),
    ...
    def show_barplot(self):
        self.barplot = self._show_pic_by_height(
            self.window,
            os.path.join(
                self.system_config.repository_dir_base, "media", "tmp", "fb_barplot.png"
            ),
    ...
    def show_smiley(self):
        self.smiley.setImage(
            os.path.join(self.images_dir_base, "smiley_transparent.png")
        )
        self.smiley.draw()
        self.window.flip()
    ...
    def show_eyes_open_close(self, show_eyes: str):
        if show_eyes.lower() == "open":
            ...
            print(
                "Error : Argument of show_eyes_open_close() has to be 'open' or 'close'."
            )
            sys.exit()
        self.window.flip()
    ...
    def hide_eyes_open_close(self, show_eyes: str):
        if show_eyes.lower() == "open":
            ...
            print(
                "Error : Argument of hide_eyes_open_close() has to be 'open' or 'close'."
            )
            sys.exit()
        self.window.flip()
    ...
    def show_gif(self, n_times: int = 1, n_frames: int = 77, sleep_time: float = 0.1):
        for i in range(0, n_times):
            for file in range(0, n_frames):
                ...
                self.dancing_gif.setImage(
                    os.path.join(
                        self.images_dir_base, "dancing_gif", str(file) + ".gif"
                    )
                )
